...
import {
  Calendar, ChevronLeft, ChevronRight, Printer, Download, RefreshCw,
  TrendingUp, CreditCard, Banknote, Building2, ReceiptText, CheckCircle2,
  XCircle, Clock, BarChart3, Layers, UserCheck, Users, Receipt,
  AlertTriangle, ArrowDownLeft, History, ChevronDown, ChevronUp,
} from "lucide-react";
import {
...
interface MonthDay {
  date: string; dayNum: number; ordersCount: number; netSales: number;
...
function KpiCard({ label, value, sub, icon, color }: {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 p-4 flex flex-col gap-2 shadow-sm">
      ...
      <div className="text-2xl font-bold text-gray-900 tabular-nums">${fmt(value)}</div>
...
function PaymentRow({ label, amount, count, children }: {
  label: string; amount: number; count?: number; children?: React.ReactNode;
}) {
    ...
    <div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="font-semibold tabular-nums text-gray-900">${fmt(amount)}</span>
          {children && (open ? <ChevronUp size={12} className="text-gray-400" /> : <ChevronDown size={12} className="text-gray-400" />)}
        </div>
      </div>
      {open && children && (
  ...
  return (
    <tr className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
      <td className="py-2.5 pl-4 pr-2 text-sm font-medium text-gray-700">
        <span className="inline-flex items-center gap-1"><span className="text-gray-400 text-xs">$</span>{denom.label}</span>
      </td>
      ...
      <td className="py-2.5 pl-2 pr-4 text-right text-sm font-semibold tabular-nums text-gray-900">${fmt(total)}</td>
    </tr>
  );
}
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="grid grid-cols-7">
        ...
        {days.map(day => {
          const intensity = day.netSales > 0 ? Math.max(0.08, day.netSales / maxSales) : 0;
          const isSelected = day.date === selectedDate;
          return (
            <button key={day.date} onClick={() => day.netSales > 0 && onSelectDate(day.date)}
              ...
              {day.netSales > 0 && !isSelected && (
                <div className="absolute inset-0 bg-emerald-400" style={{ opacity: intensity * 0.18 }} />
              )}
              <span className={`text-xs font-semibold z-10 ${isSelected ? "text-blue-700" : day.isToday ? "text-amber-700" : "text-gray-600"}`}>
                {day.dayNum}
              </span>
              {day.netSales > 0 && (
...
export default function POSClosingReport() {
  ...
  const monthTotals = monthlyData?.totals ?? { netSales: 0, ordersCount: 0, activeDays: 0, avgPerDay: 0 };
    ...
    <div className="min-h-screen bg-[#F6F7FA] font-sans">
      ...
      <div className="no-print bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[15px] font-bold text-gray-900">POS Closing Report</h1>
        ...
        <div className="flex flex-col gap-4 no-print">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <h2 className="text-sm font-bold text-gray-900">{MONTH_NAMES[calMonth]} {calYear}</h2>
            ...
            {isMonthLoading ? (
              ...
              <CalendarGrid year={calYear} month={calMonth} days={monthDays}
                selectedDate={selectedDate} onSelectDate={setSelectedDate} />
            )}
            <div className="flex items-center gap-4 mt-3 text-[10px] text-gray-400">
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded-sm bg-amber-100 border border-amber-200 inline-block" /> Today
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded-sm bg-blue-100 border border-blue-300 inline-block" /> Selected
              </span>
              <span className="flex items-center gap-1">
                <span className="w-2.5 h-2.5 rounded-sm bg-gray-100 inline-block" /> No sales
              </span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-gray-700 uppercase tracking-wide">{MONTH_NAMES[calMonth]} Summary</span>
            </div>
            <div className="flex flex-col gap-2.5">
              {[
                ["Total Revenue",  `$${fmt(monthTotals.netSales)}`],
                ["Total Orders",   (monthTotals.ordersCount || 0).toLocaleString()],
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-gray-700 uppercase tracking-wide">Quick Navigation</span>
            </div>
          ...
          {report && !report.empty && (
            <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-4 text-white shadow-sm">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-[11px] font-semibold opacity-70 uppercase tracking-wide">Selected Day</span>
              </div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm px-6 py-4">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">Session: {report.sessionName} · Cashier: {report.cashierName}</p>
                  </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-50 grid grid-cols-5 gap-3">
                  ...
                  <StatPill label="Shift"      value={`#${report.shiftNumber ?? 1}`} />
                  <StatPill label="Clock In"   value={fmtTime(report.clockIn)} />
                  <StatPill label="Clock Out"  value={fmtTime(report.clockOut)} />
                  <StatPill label="Paid Hours" value={report.paidHours != null ? `${report.paidHours}h` : "—"} />
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-xl px-6 py-4 flex items-center justify-between shadow-sm">
                <div>
                  <p className="text-slate-400 text-xs font-medium uppercase tracking-wide">Cash Owed to House</p>
                  <p className="text-white text-2xl font-bold tabular-nums mt-0.5">${fmt(report.cashOwedToHouse ?? 0)}</p>
                </div>
                <div className="text-right">
                  <p className="text-slate-400 text-xs font-medium uppercase tracking-wide">Total Tips</p>
                  <p className="text-emerald-400 text-2xl font-bold tabular-nums mt-0.5">${fmt(report.tips?.total ?? 0)}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                ...
                <KpiCard label="Sales / Guest"  value={report.salesPerGuest ?? 0} sub={`${report.guestsCount ?? "?"} guests`} icon={<Users size={14} className="text-violet-600" />} color="bg-violet-50" />
                <KpiCard label="Sales / Check"  value={report.avgOrderValue} sub="per check"                  icon={<BarChart3  size={14} className="text-amber-600"   />} color="bg-amber-50" />
              </div>

              {/* ── Stats row ── */}
              <div className="grid grid-cols-4 gap-3">
                <StatPill label="Checks"       value={String(report.ordersCount)} />
                <StatPill label="Guests"       value={String(report.guestsCount ?? "—")} />
                <StatPill label="Avg Tip %"    value={`${report.tips?.avgPct ?? 0}%`} />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <SectionHeader title="Sales Breakdown" icon={<ReceiptText size={13} />} />
                  <div className="flex flex-col gap-0">
                    {[
                      ...
                      { label: "Comps",             value: -(report.comps?.total ?? 0), cls: "text-red-500 font-medium" },
                    ].map(({ label, value, cls }) => (
                    ...
                    <div className="flex justify-between py-2 text-sm font-bold border-t border-gray-100">
                      ...
                      <span className="text-gray-900 tabular-nums">${fmt(report.netSales + report.tax)}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  ...
                  {(report.comps?.itemComps?.length ?? 0) > 0 && (
                    <div className="mb-3">
                      <p className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold mb-1">Item Comps</p>
                      {report.comps!.itemComps.map((c: any, i: number) => (
                        <div key={i} className="flex justify-between py-1.5 border-b border-gray-50 text-xs">
                          ...
                          <div className="flex gap-3">
                            <span className="text-gray-400">${fmt(c.base)}</span>
                      ...
                      <div className="flex justify-between py-1.5 text-xs font-semibold text-gray-700">
                        ...
                        <span className="tabular-nums">${fmt(report.comps!.totalItemComps)}</span>
                      </div>
                    </div>
                  )}
                  {/* Check comps */}
                  {(report.comps?.checkComps?.length ?? 0) > 0 && (
                    <div>
                      <p className="text-[10px] text-gray-400 uppercase tracking-wide font-semibold mb-1">Check Comps</p>
                      ...
                      <div className="flex justify-between py-1.5 text-xs font-semibold text-gray-700">
                        ...
                        <span className="tabular-nums">${fmt(report.comps!.totalCheckComps)}</span>
                      </div>
                    </div>
                  )}
                  {/* Grand total comps */}
                  <div className="flex justify-between py-2.5 text-sm font-bold border-t-2 border-gray-200 mt-2">
                    ...
                    <span className="text-red-600 tabular-nums">${fmt(report.comps?.total ?? 0)}</span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Payments */}
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <SectionHeader title="Payments" icon={<CreditCard size={13} />} />
                  <div className="flex flex-col gap-1 divide-y divide-gray-50">
                    <PaymentRow label="Cash" amount={report.payments.cash.amount} count={report.payments.cash.count} />
                    <PaymentRow label="Credit" amount={0} count={0} />
                    <PaymentRow label="Credit Card" amount={report.payments.card.amount} count={report.payments.card.count}
                      children={report.payments.card.breakdown?.map((b: any) => (
                        <SubRow key={b.label} label={b.label} amount={b.amount} count={b.count} />
                      ))}>
                    </PaymentRow>
                    <PaymentRow label="Cheque" amount={report.payments.cheque.amount} count={report.payments.cheque.count} />
                    <PaymentRow label="Bank Transfer" amount={report.payments.bank.amount} count={report.payments.bank.count} />
                    {report.payments.houseAccount.amount > 0 && (
                      <PaymentRow label="House Account" amount={report.payments.houseAccount.amount} count={report.payments.houseAccount.count}
                        children={report.payments.houseAccount.accounts?.map((a: any) => (
                          <SubRow key={a.name} label={a.name} amount={a.amount} count={a.count} />
                        ))}>
                      </PaymentRow>
                    )}
                    {report.payments.other.amount > 0 && (
                      <PaymentRow label="Other" amount={report.payments.other.amount} />
                    )}
                  </div>
                  <div className="mt-3 pt-3 border-t-2 border-gray-200 flex justify-between text-sm font-bold">
                    ...
                    <span className="tabular-nums text-gray-900">${fmt(report.payments.total)}</span>
                  </div>
                </div>
                  ...
                  <div className="flex flex-col gap-1 divide-y divide-gray-50">
                    {report.tips?.breakdown?.length > 0 ? (
                      report.tips.breakdown.map((t: any) => (
                        <div key={t.method} className="flex justify-between py-2 text-sm">
                          ...
                          <span className="font-semibold tabular-nums text-gray-900">${fmt(t.amount)}</span>
                        </div>
                      ))
                    ) : (
                      <p className="text-xs text-gray-400 py-3">No tips recorded</p>
                    )}
                  </div>
                  <div className="mt-3 pt-3 border-t-2 border-gray-200 flex justify-between text-sm font-bold">
                    <span className="text-gray-900">Total Tips</span>
                    <span className="tabular-nums text-emerald-700">${fmt(report.tips?.total ?? 0)}</span>
                  </div>

                  {/* Voids */}
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <SectionHeader title="Voids" icon={<AlertTriangle size={13} />} />
                    <div className="flex justify-between text-sm py-1">
                      <span className="text-gray-600">Void Count</span>
                      <span className="font-semibold text-gray-900">{report.voids?.count ?? 0} voids</span>
                    </div>
                    <div className="flex justify-between text-sm py-1">
                      <span className="text-gray-600">Void Amount</span>
                      <span className="font-semibold text-red-500">${fmt(report.voids?.amount ?? 0)}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {(report.payouts?.items?.length ?? 0) > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <SectionHeader title="Payouts" icon={<ArrowDownLeft size={13} />} />
                  <div className="flex flex-col gap-0">
                    {report.payouts.items.map((p: any, i: number) => (
                      <div key={i} className="flex justify-between py-2.5 border-b border-gray-50 text-sm">
                        <span className="text-gray-600">Cash to {report.cashierName} — {p.description}</span>
                        <span className="font-semibold tabular-nums text-gray-900">${fmt(p.amount)}</span>
                      </div>
                    ))}
                    <div className="flex justify-between py-3 text-sm font-bold border-t-2 border-gray-200 mt-1">
                      <span className="text-gray-900">Total Cashier Payouts</span>
                      <span className="text-red-500 tabular-nums">${fmt(report.payouts.total)}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                ...
                <div className="flex flex-col gap-0 max-w-sm">
                  <div className="flex justify-between py-2.5 border-b border-gray-50 text-sm">
                    ...
                    <span className="font-semibold text-gray-900 tabular-nums">+ ${fmt(report.payments.cash.amount)}</span>
                  </div>
                  <div className="flex justify-between py-2.5 border-b border-gray-50 text-sm">
                    <span className="text-gray-600">Pay-outs</span>
                    <span className="font-semibold text-red-500 tabular-nums">- ${fmt(report.payouts?.total ?? 0)}</span>
                  </div>
                  <div className="border-t border-dashed border-gray-200 my-1" />
                  <div className="flex justify-between py-3 text-sm font-bold">
                    <span className="text-gray-900">Cash Owed</span>
                    <span className="text-gray-900 tabular-nums">${fmt(report.cashOwedToHouse ?? 0)}</span>
                  </div>
                </div>
              </div>
              ...
              {report.topProducts?.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <SectionHeader title="Top Products" icon={<Layers size={13} />} />
                  <div className="flex flex-col gap-0">
                    {report.topProducts.slice(0, 8).map((p: any, i: number) => (
                      <div key={p.name} className="flex items-center justify-between py-2.5 border-b border-gray-50 text-sm">
                        <div className="flex items-center gap-3">
                          <span className="w-5 h-5 rounded-full bg-gray-100 text-[10px] font-bold text-gray-500 flex items-center justify-center">{i + 1}</span>
                          <span className="text-gray-800 font-medium">{p.name}</span>
                        </div>
                        <div className="flex items-center gap-4 text-right">
                          <span className="text-gray-400 text-xs">qty: {p.qty}</span>
                          <span className="font-semibold tabular-nums text-gray-900">${fmt(p.revenue)}</span>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide">Cash Register Count</h3>
                  <div className="flex flex-col items-end gap-1">
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setDenomEditable(v => !v)}
                      ...
                      <button onClick={handleSubmitCashCount} disabled={submitting}
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="flex flex-col gap-3">
                    {[
                      { icon: <Clock size={13} className="text-gray-400" />, label: "Opening Balance", value: `$${fmt(report.openingBalance)}`, cls: "" },
                      { icon: <Banknote size={13} className="text-emerald-500" />, label: "Cash Sales", value: `+$${fmt(report.payments.cash.amount)}`, cls: "text-emerald-700" },
                      { icon: <XCircle size={13} className="text-red-400" />, label: "Cash Refunds (est.)", value: `-$${fmt(report.refunds * 0.45)}`, cls: "text-red-500" },
                      { icon: <ArrowDownLeft size={13} className="text-orange-400" />, label: "Payouts", value: `-$${fmt(report.payouts?.total ?? 0)}`, cls: "text-orange-600" },
                    ...
                    <div className="flex justify-between items-center py-3 font-bold text-sm border-t-2 border-gray-200">
                      ...
                      <span className="tabular-nums text-gray-900">${fmt(report.expectedClosingBalance)}</span>
                    </div>
                    <div className="flex justify-between items-center py-2.5 text-sm">
                      <span className="text-gray-600">Counted (below)</span>
                      <span className={`font-bold tabular-nums ${Math.abs(denomTotal - report.expectedClosingBalance) < 1 ? "text-emerald-700" : "text-red-500"}`}>
                        ${fmt(denomTotal)}
                      </span>
                    </div>
                    <div className={`flex justify-between items-center py-2 rounded-lg px-3 text-sm ${Math.abs(denomTotal - report.expectedClosingBalance) < 1 ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
                      <span className="font-medium">Difference</span>
                      <span className="font-bold tabular-nums">
                        {denomTotal - report.expectedClosingBalance >= 0 ? "+" : ""}${fmt(denomTotal - report.expectedClosingBalance)}
                      </span>
                    </div>
                    <textarea placeholder="Notes (optional)" value={cashCountNotes}
                    ...
                    <table className="w-full">
                      <thead>
                        <tr className="border-b border-gray-100">
                          <th className="text-left text-[10px] font-bold text-gray-400 uppercase tracking-wide pb-2 pl-4">Bill / Coin</th>
                          <th className="text-center text-[10px] font-bold text-gray-400 uppercase tracking-wide pb-2">Count</th>
                          <th className="text-right text-[10px] font-bold text-gray-400 uppercase tracking-wide pb-2 pr-4">Total</th>
                        </tr>
                      </thead>
                      <tbody>
                        {denominations.map((d, i) => (
                          <DenominationRow key={d.label} denom={d} editable={denomEditable}
                            onChange={count => updateDenominations(denominations.map((item, idx) =>
                              idx === i ? { ...item, count } : item))} />
                        ))}
                        <tr className="border-t-2 border-gray-200">
                          <td colSpan={2} className="py-3 pl-4 text-sm font-bold text-gray-900">Total Cash</td>
                          <td className="py-3 pr-4 text-right text-sm font-bold text-emerald-700 tabular-nums">${fmt(denomTotal)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              {/* ── Cashier Sign-Off ── */}
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-4">Cashier Sign-Off</h3>
