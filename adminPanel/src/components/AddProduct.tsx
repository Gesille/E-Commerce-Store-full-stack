...
import { useCreateProductMutation, useGetOdooLocationsQuery } from "@/redux/product/productApi";
import toast from "react-hot-toast";
import { useGetCategoriesQuery } from "@/redux/category/categoryApi";
import { BarcodeSticker } from "./BarcodeSticker";

const colors = [
  "red", "orange", "amber", "yellow", "blue",
  "purple", "cyan", "black", "white", "gray",
] as const;
...
const formSchema = z.object({
  ...
  // Now a single locationId picked from dropdown
...
const AddProduct = () => {
  ...
  const { data: odooLocations = [], isLoading: locationsLoading } = useGetOdooLocationsQuery();
  ...
  const calculatedFinalPrice =
    (Number(supplierPrice) + Number(shippingCost)) * (XCD_RATES[currency] ?? 2.7);
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      await createProduct({
        ...
        locationId:    data.locationId,   // ← real Odoo location ID
        supplierId:    data.supplierId,   // ← invoice number, now actually sent
  ...
  const generateEAN13 = () => {
    ...
    const check =
      (10 - (digits.reduce((sum, d, i) => sum + d * (i % 2 === 0 ? 1 : 3), 0) % 10)) % 10;
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Section title="Product Identity">
              ...
              <FormField control={form.control} name="barcode" render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-2 text-indigo-700">Barcode</FormLabel>
            ...
            <Section title="Pricing & Cost">
              ...
              <FormField control={form.control} name="supplierPrice" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input type="number" min={0} step="0.01" {...field} className="bg-white border-emerald-200 rounded-lg" /></FormControl>
                  <FormDescription>Cost price (<code>standard_price</code>)</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <div className="rounded-lg bg-emerald-50 border border-emerald-100 px-4 py-3 space-y-3">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <Checkbox checked={useFinalPriceAsSale} onCheckedChange={(c) => setUseFinalPriceAsSale(!!c)} />
            ...
            <Section title="Inventory">
              <FormField control={form.control} name="quantity" render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>Applied via <code>stock.quant</code></FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
            ...
            {/* ── 4. Location — real Odoo dropdown ── */}
            <Section title="Storage Location">
              <FormField control={form.control} name="locationId" render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-2 text-sky-700"><MapPin size={14} /> Location *</FormLabel>
                  <Select
                  ...
                  <FormDescription>
                    Real warehouse location from Odoo (e.g. WH/Stock)
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
            ...
            <Section title="Supplier">
              <FormField control={form.control} name="supplierId" render={({ field }) => (
                <FormItem>
                  <FormLabel className="flex items-center gap-2 text-orange-600"><User size={14} /> Supplier Invoice #</FormLabel>
                  <FormControl>
                    <Input {...field} placeholder="e.g. INV-2024-001" className="bg-white border-orange-200 rounded-lg" />
                  </FormControl>
                  <FormDescription>Saved as <code>product_code</code> in Odoo supplier info</FormDescription>
                  <FormMessage />
                </FormItem>
              )} />
              ...
              <FormField control={form.control} name="supplierName" render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="e.g. Acme Corp" className="bg-white border-orange-200 rounded-lg" />
                  </FormControl>
