...
export const UpdateProductModal = ({
  ...
  useEffect(() => {
    ...
    setSupplierId(
      (p as any).supplierInvoiceNumber ?? (p as any).supplierId ?? "",
    );
    ...
    setMaterials(
      Array.isArray(p.attributes?.materials) ? p.attributes.materials : [],
    );
    setImage(null);
  ...
  const generateEAN13 = () => {
    ...
    const check =
      (10 -
        (digits.reduce((s, d, i) => s + d * (i % 2 === 0 ? 1 : 3), 0) % 10)) %
      10;
  ...
  const fieldClass =
    "bg-white border rounded-lg focus-visible:ring-2 focus-visible:ring-offset-0";
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-8">
          ...
          <Section title="Pricing & Cost">
            <div className="space-y-1.5">
              ...
              <Select
                value={currency}
                onValueChange={(v) => setCurrency(v as "USD" | "EUR")}
              >
