...
export const taxApi = apiSlice.injectEndpoints({
  ...
  endpoints: (builder) => ({
    ...
    // Export: this returns a file, so we handle it differently.
    // Use a direct fetch/window.open in the component instead of RTK Query.
    // Helper function exported below.
  }),
});
