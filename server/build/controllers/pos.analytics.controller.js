...
function orderDomain(from, to, extra = []) {
    return [
...
// ─── KPI Summary
export const getKpiSummary = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId);
    const { from, to, prevFrom, prevTo } = getPeriodRange(period);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    ...
    const [currentOrders, prevOrders, refundOrders, prevRefunds] = await Promise.all([
        odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["amount_total", "lines"], limit: 5000 }),
        odooRequest("pos.order", "search_read", [orderDomain(prevFrom, prevTo, sessionFilter)], { fields: ["amount_total"], limit: 5000 }),
    ...
    const totalRevenue = currentOrders.reduce((s, o) => s + (o.amount_total ?? 0), 0);
    const prevRevenue = prevOrders.reduce((s, o) => s + (o.amount_total ?? 0), 0);
    const orderCount = currentOrders.length;
    const prevCount = prevOrders.length;
    const avgOrder = orderCount > 0 ? totalRevenue / orderCount : 0;
    const prevAvg = prevCount > 0 ? prevRevenue / prevCount : 0;
    const totalRefunds = Math.abs(refundOrders.reduce((s, o) => s + (o.amount_total ?? 0), 0));
    const prevRefundAmt = Math.abs(prevRefunds.reduce((s, o) => s + (o.amount_total ?? 0), 0));
    const pctDelta = (curr, prev) => prev === 0 ? 0 : Math.round(((curr - prev) / prev) * 1000) / 10;
...
export const getRevenueChart = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to, prevFrom, prevTo } = getPeriodRange(period);
    const [currentOrders, prevOrders] = await Promise.all([
        odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["date_order", "amount_total"], limit: 5000 }),
        odooRequest("pos.order", "search_read", [orderDomain(prevFrom, prevTo, sessionFilter)], { fields: ["date_order", "amount_total"], limit: 5000 }),
    ]);
...
export const getTopProducts = CatchAsyncError(async (req, res, next) => {
    ...
    const orders = await odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["lines"], limit: 5000 });
    const orderIds = orders.map((o) => o.id);
    if (!orderIds.length) {
        return res.status(200).json({ status: "success", period, products: [] });
    }
    const lines = await odooRequest("pos.order.line", "search_read", [[["order_id", "in", orderIds]]], { fields: ["product_id", "qty", "price_subtotal_incl"], limit: 20000 });
    ...
    for (const line of lines) {
        ...
        if (!productMap[id])
            productMap[id] = { name, revenue: 0, orders: 0 };
...
export const getRecentOrders = CatchAsyncError(async (req, res, next) => {
    const configId = Number(req.query.configId);
    ...
    const result = orders.map((o) => {
        ...
        let status = "paid";
        if (o.state === "cancel" || o.amount_total < 0)
            status = "refund";
...
// ─── Payment Methods ──────────────────────────────────────────────────────────
export const getPaymentMethodsSplit = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId) || 0;
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to } = getPeriodRange(period);
    // ─── 1. Fetch orders ───────────────────────────────────────────────────
    const orders = await odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["id"], limit: 5000 });
    const orderIds = orders.map((o) => o.id);
    if (!orderIds.length) {
    ...
    try {
        ...
        payments = await odooRequest("pos.payment", "search_read", [[["pos_order_id", "in", orderIds]]], { fields: ["payment_method_id", "amount"], limit: 20000 });
    }
    catch {
        // Odoo 14/15 uses "order_id"
        payments = await odooRequest("pos.payment", "search_read", [[["order_id", "in", orderIds]]], { fields: ["payment_method_id", "amount"], limit: 20000 });
    }
    if (!payments.length) {
        return res.status(200).json({
            status: "success",
    ...
    const methods = Object.entries(methodMap)
        ...
        percentage: total > 0 ? Math.round((m.amount / total) * 10000) / 100 : 0,
    }));
    // ─── 5. Respond ────────────────────────────────────────────────────────
    return res.status(200).json({
        status: "success",
...
export const getCategoryBreakdown = CatchAsyncError(async (req, res, next) => {
    ...
    const lines = await odooRequest("pos.order.line", "search_read", [[["order_id", "in", orderIds]]], { fields: ["product_id", "price_subtotal_incl", "qty"], limit: 20000 });
    const productIds = [
        ...new Set(lines.map((l) => l.product_id?.[0]).filter(Boolean)),
    ];
    // Fetch product categories
    const products = await odooRequest("product.product", "search_read", [[["id", "in", productIds]]], { fields: ["id", "categ_id"], limit: 1000 });
    ...
    for (const line of lines) {
        ...
        if (!categoryMap[cat])
            categoryMap[cat] = { value: 0, orders: 0 };
...
export const getStaffPerformance = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId) || 0;
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to, prevFrom, prevTo } = getPeriodRange(period);
    ...
    const [currentOrders, prevOrders] = await Promise.all([
        odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["user_id", "amount_total"], limit: 5000 }),
        odooRequest("pos.order", "search_read", [orderDomain(prevFrom, prevTo, sessionFilter)], { fields: ["user_id", "amount_total"], limit: 5000 }),
    ]);
    ...
    for (const o of currentOrders) {
        ...
        if (!currMap[uid])
            currMap[uid] = { name, sales: 0, txn: 0 };
...
export const getRevenueTarget = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to } = getPeriodRange(period);
    const orders = await odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["amount_total"], limit: 5000 });
    const current = orders.reduce((s, o) => s + (o.amount_total ?? 0), 0);
...
export const getSessionInfo = CatchAsyncError(async (req, res, next) => {
    const configId = Number(req.query.configId);
    if (!configId)
    ...
    if (!session) {
        return res
            .status(200)
            .json({ status: "success", session: null, shift: null });
    }
    ...
    const cashier = activeShift ? activeShift.cashierId : null;
    res.status(200).json({
        status: "success",
        session: {
            id: session.id,
...
export const getLowStockAlerts = CatchAsyncError(async (req, res, next) => {
    ...
    const products = await odooRequest("product.template", "search_read", [[["active", "=", true]]], { fields: ["id", "name", "default_code", "list_price"] });
    if (!products.length) {
    ...
    for (const q of quants) {
        ...
        if (tmplId)
            stockMap[tmplId] = (stockMap[tmplId] ?? 0) + q.quantity;
    }
...
export const getPeakHoursHeatmap = CatchAsyncError(async (req, res, next) => {
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
...
export const getTableStatus = CatchAsyncError(async (req, res, next) => {
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    // Fetch all tables (restaurant.table) for this config
    const tables = await odooRequest("restaurant.table", "search_read", [configId ? [["pos_config_id", "=", configId]] : []], {
        fields: ["id", "name", "seats", "state", "current_order_id"],
        limit: 100,
    });
    // Get open orders on those tables to compute duration
    const tableOrderIds = tables
        .map((t) => t.current_order_id?.[0])
        .filter(Boolean);
    let orderMap = {};
    if (tableOrderIds.length) {
        const openOrders = await odooRequest("pos.order", "search_read", [[["id", "in", tableOrderIds]]], {
    ...
    const result = tables.map((t) => {
        ...
        const status = t.state === "occupied" ? "occupied" : "available";
        return {
            id: t.name,
            status,
            duration,
...
export const getDiscounts = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to } = getPeriodRange(period);
    ...
    for (const l of lines) {
        ...
        const lineDiscount = (l.price_unit * l.qty * l.discount) / 100;
        if (!discountMap[key])
            discountMap[key] = { uses: 0, saved: 0, type: "Discount" };
    ...
    for (const l of couponLines) {
        ...
        if (!discountMap[key])
            discountMap[key] = { uses: 0, saved: 0, type: "Promotion" };
        discountMap[key].uses += 1;
...
export const getCustomerInsights = CatchAsyncError(async (req, res, next) => {
    const period = req.query.period || "today";
    const configId = Number(req.query.configId);
    const sessionFilter = configId ? [["config_id", "=", configId]] : [];
    const { from, to } = getPeriodRange(period);
    const orders = await odooRequest("pos.order", "search_read", [orderDomain(from, to, sessionFilter)], { fields: ["partner_id", "amount_total"], limit: 5000 });
    // Segment: orders with a linked partner = known customer
    const withPartner = orders.filter((o) => o.partner_id);
    const withoutPartner = orders.filter((o) => !o.partner_id);
    // Identify "new" vs "returning" by checking if partner had any order BEFORE this period
    const partnerIds = [
        ...new Set(withPartner.map((o) => o.partner_id[0])),
    ];
    ...
    if (partnerIds.length) {
        ...
        for (const o of prevOrders)
            returningIds.add(o.partner_id[0]);
    }
    const newCustomers = withPartner.filter((o) => !returningIds.has(o.partner_id[0])).length;
    const returningCustomers = withPartner.filter((o) => returningIds.has(o.partner_id[0])).length;
    // Top spender
    const spendMap = {};
    for (const o of withPartner) {
        const id = o.partner_id[0];
        if (!spendMap[id])
            spendMap[id] = { name: o.partner_id[1], total: 0 };
