...
// ─── helpers ────────────────────────────────────────────────────────────────
function buildDomain(dateFrom, dateTo) {
    return [
...
export const getMonthlySalesReport = CatchAsyncError(async (req, res, next) => {
...
export const getDailySalesReport = CatchAsyncError(async (req, res, next) => {
    const year = parseInt(req.query.year) || new Date().getFullYear();
...
export const getProductSalesReport = CatchAsyncError(async (req, res, next) => {
...
export const getInventoryMovementReport = CatchAsyncError(async (req, res, next) => {
    const { date_from, date_to } = req.query;
    if (!date_from || !date_to) {
        return next(new ErrorHandler("date_from and date_to are required (YYYY-MM-DD)", 400));
    }
    ...
    for (const move of moves) {
        ...
        const destName = move.location_dest_id[1] ?? "";
        const srcName = move.location_id[1] ?? "";
        if (!summary[pid]) {
        ...
        if (isOut)
            summary[pid].qty_out += move.product_uom_qty;
        if (isIn)
            summary[pid].qty_in += move.product_uom_qty;
    }
    res.json({
        success: true,
        date_from,
...
// export
// ─── Monthly Export ──────────────────────────────────────────────────────────
export const exportMonthlySalesReport = CatchAsyncError(async (req, res, next) => {
    const year = parseInt(req.query.year) || new Date().getFullYear();
    ...
    if (format === "excel") {
        ...
        const html = buildHtmlTable(`Monthly Sales Report — ${year}`, ["Month", "Orders", "Revenue", "Revenue Untaxed"], tableRows);
        await streamPdf(res, html, `monthly-sales-${year}.pdf`);
    }
});
// ─── Daily Export ────────────────────────────────────────────────────────────
export const exportDailySalesReport = CatchAsyncError(async (req, res, next) => {
    const year = parseInt(req.query.year) || new Date().getFullYear();
    const month = parseInt(req.query.month) || new Date().getMonth() + 1;
    const format = req.query.format || "excel";
    const dateFrom = `${year}-${String(month).padStart(2, "0")}-01`;
    ...
    for (const o of orders) {
        ...
        if (days[key]) {
            days[key].orders++;
            days[key].revenue += o.amount_total;
        }
    }
    ...
    if (format === "excel") {
        ...
        const html = buildHtmlTable(`Daily Sales — ${year}/${String(month).padStart(2, "0")}`, ["Date", "Orders", "Revenue"], tableRows);
        await streamPdf(res, html, `daily-sales-${year}-${month}.pdf`);
    }
});
// ─── Product Export ──────────────────────────────────────────────────────────
export const exportProductSalesReport = CatchAsyncError(async (req, res, next) => {
    const { date_from, date_to, format = "excel" } = req.query;
    if (!date_from || !date_to)
        return next(new ErrorHandler("date_from and date_to required", 400));
    const lines = await odooRequest("sale.order.line", "search_read", [[
            ["order_id.state", "in", ["sale", "done"]],
            ["order_id.date_order", ">=", `${date_from} 00:00:00`],
            ["order_id.date_order", "<=", `${date_to} 23:59:59`],
    ...
    for (const l of lines) {
        ...
        if (!products[pid])
            products[pid] = { product_id: pid, product_name: l.product_id[1], qty_sold: 0, revenue: 0, orders: new Set() };
    ...
    if (format === "excel") {
        ...
        const html = buildHtmlTable(`Product Sales — ${date_from} to ${date_to}`, ["#", "Product", "Units", "Orders", "Revenue"], tableRows);
        await streamPdf(res, html, `product-sales.pdf`);
    }
});
// ─── Inventory Export ────────────────────────────────────────────────────────
export const exportInventoryReport = CatchAsyncError(async (req, res, next) => {
    const { date_from, date_to, format = "excel" } = req.query;
    if (!date_from || !date_to)
        return next(new ErrorHandler("date_from and date_to required", 400));
    const moves = await odooRequest("stock.move", "search_read", [[
    ...
    for (const m of moves) {
        ...
        if (!summary[pid])
            summary[pid] = { product_id: pid, product_name: m.product_id[1], qty_in: 0, qty_out: 0 };
        const dest = (m.location_dest_id[1] ?? "").toLowerCase();
        const src = (m.location_id[1] ?? "").toLowerCase();
        if (dest.includes("customer") || dest.includes("virtual"))
            summary[pid].qty_out += m.product_uom_qty;
        if (src.includes("vendor") || src.includes("supplier"))
            summary[pid].qty_in += m.product_uom_qty;
    }
    ...
    if (format === "excel") {
        ...
        const html = buildHtmlTable(`Inventory Movements — ${date_from} to ${date_to}`, ["Product", "In", "Out", "Net"], tableRows);
...
async function streamPdf(res, html, filename) {
