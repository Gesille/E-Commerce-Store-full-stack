...
// Internal Helpers
// ─────────────────────────────────────────────────────────────────────────────
async function resolveCashier(cashierId) {
    ...
    if (!user.odooPartnerId)
        throw new ErrorHandler("Cashier has no linked Odoo partner (odooPartnerId missing)", 422);
    return user;
}
async function fetchOpenOdooSession(configId) {
...
async function computeShiftTotalsFromOrders(shiftId) {
...
async function populateShift(shiftId) {
    return CashierShiftLog.findById(shiftId)
        .populate("cashierId", "name email role")
        .lean();
}
export const openSession = CatchAsyncError(async (req, res, next) => {
    ...
    if (!cashier)
        return;
    const existing = await odooRequest("pos.session", "search_read", [
        [
    ...
    if (existing?.length) {
        ...
        if (existingSession.state === "opening_control") {
            return res.status(200).json({
                ...
                message: "An existing session is awaiting opening balance confirmation",
            });
        }
    ...
    if (!session) {
        return next(new ErrorHandler("Session was created but could not be retrieved", 500));
    }
...
export const confirmOpeningBalance = CatchAsyncError(async (req, res, next) => {
    ...
    let currentSessions = [];
    try {
        currentSessions = await odooRequest("pos.session", "read", [[sessionId]], {
            fields: [
                "id",
                "name",
                "state",
        ...
        console.log("[POS] FULL SESSION:", JSON.stringify(currentSessions, null, 2));
    }
    catch (err) {
    ...
    if (current.state !== "opening_control") {
        return next(new ErrorHandler(`Invalid session state: ${current.state}`, 409));
    }
    ...
    // ── Open session ─────────────────────────────
    try {
        const result = await odooRequest("pos.session", "action_pos_session_open", [[sessionId]]);
        console.log("[POS] OPEN RESULT:", result);
    }
    catch (err) {
        console.error("[POS] action_pos_session_open ERROR:", err);
        return next(new ErrorHandler(err?.message || "Failed to open POS session", 500));
    }
    ...
    // ── Final check ──────────────────────────────
    const finalVerify = await odooRequest("pos.session", "read", [[sessionId]], { fields: ["id", "name", "state", "cash_register_balance_start"] });
    const finalSession = finalVerify?.[0];
    console.log("[POS] FINAL SESSION:", finalSession);
    if (!finalSession || finalSession.state !== "opened") {
        return next(new ErrorHandler(`Session failed to open. Current state: ${finalSession?.state}`, 500));
    }
    const shiftLog = await CashierShiftLog.create({
        odooSessionId: sessionId,
        cashierId: cashier._id,
...
export const closeSession = CatchAsyncError(async (req, res, next) => {
    ...
    if (session.state === "opening_control") {
        await CashierShiftLog.updateMany({ odooSessionId: sessionId, state: { $ne: "closed" } }, {
        ...
        return res.status(200).json({
            status: "success",
            message: "Session was in opening_control and has been abandoned. No orders were taken.",
            sessionId,
            cashierReport: [],
        });
    }
    ...
    const shiftLogs = await CashierShiftLog.find({ odooSessionId: sessionId })
        .populate("cashierId", "name email role")
        .lean();
    const cashierReport = await Promise.all(shiftLogs.map(async (log) => {
...
export const getActiveSession = CatchAsyncError(async (req, res, next) => {
...
// Shift Controllers
// ─────────────────────────────────────────────────────────────────────────────
export const startCashierShift = CatchAsyncError(async (req, res, next) => {
    ...
    if (existingShift) {
        return next(new ErrorHandler(`This cashier already has an ${existingShift.state} shift in the current session`, 409));
    }
    const shiftLog = await CashierShiftLog.create({
        odooSessionId: session.id,
        cashierId: cashier._id,
...
export const pauseCashierShift = CatchAsyncError(async (req, res, next) => {
    ...
    if (!shift) {
        return next(new ErrorHandler("No active shift found for this cashier in the current session", 404));
    }
    shift.state = "paused";
...
export const resumeCashierShift = CatchAsyncError(async (req, res, next) => {
    const { cashierId, reason, configId } = req.body;
    if (!cashierId)
        return next(new ErrorHandler("cashierId is required", 400));
    const session = await fetchOpenOdooSession(configId);
    ...
    if (!shift) {
        return next(new ErrorHandler("No paused shift found for this cashier in the current session", 404));
    }
    shift.state = "active";
    shift.stateHistory.push({
        toState: "active",
...
export const endCashierShift = CatchAsyncError(async (req, res, next) => {
    const { cashierId, configId } = req.body;
    if (!cashierId)
        return next(new ErrorHandler("cashierId is required", 400));
    const session = await fetchOpenOdooSession(configId);
    ...
    if (!shift) {
        return next(new ErrorHandler("No active or paused shift found for this cashier in the current session", 404));
    }
    const now = new Date();
    shift.endTime = now;
    shift.state = "closed";
...
export const getActiveShifts = CatchAsyncError(async (req, res, next) => {
    const configId = Number(req.query.configId);
    const session = await fetchOpenOdooSession(configId);
    if (!session)
        return next(new ErrorHandler("No open POS session found", 409));
...
export const createOrder = CatchAsyncError(async (req, res, next) => {
    const { cart, paymentLines, cashierId, customerId, note, configId, } = req.body;
    // ─── VALIDATION ───────────────────────────────────────────────
    if (!cart?.length) {
    ...
    if (!shift) {
        return next(new ErrorHandler("Cashier does not have an active shift.", 409));
    }
    // ─── POS CONFIG ───────────────────────────────────────────────
    const posConfig = await odooRequest("pos.config", "search_read", [[["id", "=", configId]]], { fields: ["picking_type_id"], limit: 1 });
    const pickingTypeId = posConfig?.[0]?.picking_type_id?.[0];
    if (!pickingTypeId) {
        return next(new ErrorHandler("POS picking type not configured", 500));
    }
    // ─── PICKING TYPE ─────────────────────────────────────────────
    const pickingType = await odooRequest("stock.picking.type", "search_read", [[["id", "=", pickingTypeId]]], {
    ...
    for (const item of cart) {
        const product = await odooRequest("product.product", "search_read", [[["product_tmpl_id", "=", Number(item.productId)]]], { fields: ["id", "name", "uom_id", "qty_available"], limit: 1 });
        if (!product.length) {
        ...
        if (availableQty < item.qty) {
            return next(new ErrorHandler(`"${realProduct.name}" only has ${availableQty} unit(s) left in stock`, 400));
        }
        resolvedCart.push({
            ...item,
...
export const getSessionOrders = CatchAsyncError(async (req, res, next) => {
    const sessionId = Number(req.params.sessionId);
    if (!sessionId)
        return next(new ErrorHandler("sessionId is required", 400));
    const orders = await odooRequest("pos.order", "search_read", [[["session_id", "=", sessionId]]], {
        fields: [
            "id",
            "name",
            "date_order",
...
export const getSessionReport = CatchAsyncError(async (req, res, next) => {
    const sessionId = Number(req.params.sessionId);
    if (!sessionId)
        return next(new ErrorHandler("sessionId is required", 400));
    const shiftLogs = await CashierShiftLog.find({ odooSessionId: sessionId })
        .populate("cashierId", "name email role")
        .lean();
    const cashierSummaries = await Promise.all(shiftLogs.map(async (log) => {
        const accurate = await computeShiftTotalsFromOrders(String(log._id));
        return {
            shiftId: log._id,
            cashier: log.cashierId,
...
export const getCustomers = CatchAsyncError(async (req, res, next) => {
    ...
    const customers = await odooRequest("res.partner", "search_read", [domain], { fields: ["id", "name", "phone", "email", "street"], limit: 100 });
    res
        .status(200)
...
export const getPaymentMethods = CatchAsyncError(async (req, res, next) => {
    const methods = await odooRequest("pos.payment.method", "search_read", [[["active", "=", true]]], { fields: ["id", "name", "is_cash_count"] });
    res.status(200).json({ status: "success", methods });
});
export const getPOSConfigs = CatchAsyncError(async (req, res, next) => {
    const configs = await odooRequest("pos.config", "search_read", [[["active", "=", true]]], { fields: ["id", "name", "currency_id"] });
    res.status(200).json({ status: "success", configs });
});
export const debugPOSConfig = CatchAsyncError(async (req, res) => {
...
export const createOdooInvoice = CatchAsyncError(async (req, res, next) => {
    const { odooOrderId } = req.body;
    if (!odooOrderId)
        return next(new ErrorHandler("odooOrderId is required", 400));
    const orders = await odooRequest("pos.order", "search_read", [[["id", "=", odooOrderId]]], { fields: ["id", "partner_id", "lines", "amount_total"], limit: 1 });
    const order = orders?.[0];
    if (!order)
        return next(new ErrorHandler("Order not found in Odoo", 404));
    const lines = await odooRequest("pos.order.line", "search_read", [[["id", "in", order.lines]]], { fields: ["product_id", "qty", "price_unit", "discount"] });
...
export const getPosOrders = CatchAsyncError(async (req, res, next) => {
    const page = Number(req.query.page) || 1;
    const limit = Number(req.query.limit) || 20;
    const status = req.query.status; // "paid" | "invoiced" | "nothing"
    const search = req.query.search;
    ...
    const payments = paymentIds.length
        ? await odooRequest("pos.payment", "search_read", [[["id", "in", paymentIds]]], { fields: ["id", "payment_method_id", "amount", "pos_order_id"] })
        : [];
    const paymentMap = {};
...
export const getPosOrderById = CatchAsyncError(async (req, res, next) => {
    const id = Number(req.params.id);
    const [order] = await odooRequest("pos.order", "search_read", [[["id", "=", id]]], {
        fields: [
            "name",
            "date_order",
            "state",
