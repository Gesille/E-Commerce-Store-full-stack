...
export const getCategories = async (req, res) => {
    try {
        ...
        const odooCategories = await odooRequest("product.category", "search_read", [[]], {
            fields: ["id", "name"],
        });
        // convert Odoo to same format
        const formattedOdoo = odooCategories.map((cat) => ({
            catTitle: cat.name,
        ...
        //  deduplicate
        const unique = Array.from(new Map(merged.map((item) => [item.odooCategoryId, item])).values());
