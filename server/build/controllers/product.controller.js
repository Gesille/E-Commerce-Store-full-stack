...
import { checkOdooConnection, getAllProductsService, getProductByIdService, } from "../services/product.service.js";
...
export const testOdooConnection = CatchAsyncError(async (req, res, next) => {
    try {
...
// GET ALL
export const getAllProductsFromOdoo = CatchAsyncError(async (req, res, next) => {
...
export const getProductByIdFromOdoo = async (req, res, next) => {
    try {
...
const createOrGetAttribute = async (name) => {
...
const createAttributeLines = async (productTemplateId, attributeId, valueIds) => {
...
export const createProduct = async (req, res) => {
    ...
    try {
        ...
        const variant = await odooRequest("product.product", "search_read", [[["product_tmpl_id", "=", createdProductTemplateId]]], { fields: ["id"], limit: 1 });
        ...
        if (!resolvedLocationId) {
            const locations = await odooRequest("stock.location", "search_read", [[["usage", "=", "internal"]]], { fields: ["id"], limit: 1 });
...
export const decreaseStock = async (productId, qty) => {
    // get locations
    const locations = await odooRequest("stock.location", "search_read", [], { fields: ["id", "usage"] });
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const exists = await odooRequest("product.template", "search_read", [[["id", "=", Number(id)]]], { fields: ["id", "name"] });
        ...
        const updated = await Product.findOneAndUpdate({ odooProductId: Number(id) }, {
            $set: {
                name,
                reference: reference || "",
                barcode: barcode || false,
    ...
    try {
        ...
        for (const order of orders) {
            for (const item of order.items) {
                const product = await odooRequest("product.product", "search_read", [[["id", "=", item.productId]]], {
...
export const getTopSellingProducts = CatchAsyncError(async (req, res, next) => {
    try {
        const lines = await odooRequest("sale.order.line", "search_read", [[["order_id.state", "in", ["sale", "done"]]]], {
...
export const getLowStockAlerts = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const products = await odooRequest("product.template", "search_read", [[["qty_available", "<=", threshold], ["active", "=", true]]], {
...
// Barcode
export const getProductByBarcode = CatchAsyncError(async (req, res, next) => {
    const { code } = req.params;
    const product = await odooRequest("product.template", "search_read", [["|", ["barcode", "=", code], ["default_code", "=", code]]], // ✅ OR condition
    {
