...
export const createOrder = CatchAsyncError(async (req, res, next) => {
    ...
    const itemsWithNames = await Promise.all(order.items.map(async (item) => {
        const product = await odooRequest("product.template", "search_read", [[["id", "=", item.productId]]], { fields: ["name", "default_code"], limit: 1 });
        return {
            ...item.toObject(),
    ...
    await Promise.all(adminUsers.map((adminUser) => sendMail({
...
// ===============================
export const managerConfirmOrder = CatchAsyncError(async (req, res, next) => {
    ...
    try {
        ...
        for (const item of order.items) {
            // 🔥 Get correct variant
            const variant = await odooRequest("product.product", "search_read", [[["product_tmpl_id", "=", item.productId]]], { fields: ["id", "qty_available"], limit: 1 });
        ...
        const pickings = await odooRequest("stock.picking", "search_read", [[["sale_id", "=", saleOrderId]]], { fields: ["id", "state"] });
        if (pickings.length > 0) {
            const pickingId = pickings[0].id;
            // ✅ make sure it's assigned
            await odooRequest("stock.picking", "action_assign", [[pickingId]]);
            let moves = await odooRequest("stock.move", "search_read", [[["picking_id", "=", pickingId]]], {
                fields: [
                    "id",
                    "product_id",
                    "product_uom_qty",
            ...
            const existingLines = await odooRequest("stock.move.line", "search_read", [[["picking_id", "=", pickingId]]], { fields: ["id", "quantity"] });
...
export const managerCancelOrder = CatchAsyncError(async (req, res, next) => {
    ...
    const itemsWithNames = await Promise.all(order.items.map(async (item) => {
        const product = await odooRequest("product.template", "search_read", [[["id", "=", item.productId]]], { fields: ["name"], limit: 1 });
        return {
            ...item.toObject(),
            name: product[0]?.name || `Product #${item.productId}`,
        };
...
// return product
export const returnOrderItems = CatchAsyncError(async (req, res, next) => {
    ...
    const pickings = await odooRequest("stock.picking", "search_read", [[["sale_id", "=", saleOrderId]]], { fields: ["id", "name"] });
    ...
    // 3. GET RETURN LINES
    const lines = await odooRequest("stock.return.picking.line", "search_read", [[["wizard_id", "=", returnWizardId]]], {
    ...
    const enrichedLines = await Promise.all(lines.map(async (line) => {
        const product = await odooRequest("product.product", "search_read", [[["id", "=", line.product_id[0]]]], { fields: ["id", "name"], limit: 1 });
    ...
    for (const item of itemsToReturn) {
        ...
        const line = enrichedLines.find((l) => l.productName?.toLowerCase() === item.productName?.toLowerCase());
        if (!line) {
    ...
    const result = await odooRequest("stock.return.picking", "action_create_returns", [[returnWizardId]]);
...
export const getAdminOrders = CatchAsyncError(async (req, res, next) => {
    try {
        const orders = await odooRequest("sale.order", "search_read", [[]], // all orders
        {
            fields: [
                "name", // S00051
...
export const getAdminOrderDetail = CatchAsyncError(async (req, res, next) => {
    try {
        const { id } = req.params;
        // 1️⃣ get the sale order
        const orders = await odooRequest("sale.order", "search_read", [[["id", "=", Number(id)]]], {
            fields: [
                "name",
                "date_order",
                "partner_id",
        ...
        const lines = await odooRequest("sale.order.line", "search_read", [[["order_id", "=", Number(id)]]], {
            fields: [
                "product_id",
                "name",
                "product_uom_qty",
...
// track order
export const trackOrder = CatchAsyncError(async (req, res, next) => {
    const { orderId } = req.params;
    const order = await Order.findById(orderId);
    if (!order) {
        return next(new ErrorHandler("Order not found", 404));
    ...
    // 1. sale.order state
    const sale = await odooRequest("sale.order", "search_read", [[["id", "=", order.odooSaleOrderId]]], { fields: ["state"] });
    const state = sale[0]?.state;
    // 2. stock.picking state (REAL DELIVERY)
    const picking = await odooRequest("stock.picking", "search_read", [[["origin", "=", `WEB_ORDER_${order._id}`]]], { fields: ["state"] });
...
export const getInventoryReport = async (req, res, next) => {
    try {
        const products = await odooRequest("product.template", "search_read", [[]], {
            fields: [
                "name",
                "default_code",
                "qty_available",
...
export const exportInventory = CatchAsyncError(async (req, res, next) => {
    ...
    if (format === "pdf") {
        try {
            const DocClass = jsPDF.default || jsPDF;
...
export const getMonthlyRevenue = CatchAsyncError(async (req, res, next) => {
    try {
        ...
        const orders = await odooRequest("sale.order", "search_read", [[["state", "!=", "cancel"]]], {
...
// order.controller.ts
export const getOrderStatusStats = CatchAsyncError(async (req, res, next) => {
    try {
        const orders = await odooRequest("sale.order", "search_read", [[["state", "!=", "cancel"]]], { fields: ["state"] });
        const counts = {};
...
// order.controller.ts
export const getLatestTransactions = CatchAsyncError(async (req, res, next) => {
    try {
        const orders = await odooRequest("sale.order", "search_read", [[["state", "in", ["sale", "done"]]]], {
...
export const managerCreateOrder = CatchAsyncError(async (req, res, next) => {
    ...
    const resolvedItems = await Promise.all(items.map(async (item) => {
        const product = await odooRequest("product.template", "search_read", [[["default_code", "=", item.reference]]], { fields: ["id", "name", "default_code", "qty_available"], limit: 1 });
        ...
        if (odooName !== inputName)
            return next(new ErrorHandler(`Product name mismatch for "${item.reference}": expected "${product[0].name}", got "${item.name}"`, 400));
        // 4. Check sufficient stock
        if (product[0].qty_available < item.quantity)
            return next(new ErrorHandler(`Insufficient stock for "${product[0].name}": available ${product[0].qty_available}, requested ${item.quantity}`, 400));
        return {
    ...
    for (const item of resolvedItems) {
        ...
        const variant = await odooRequest("product.product", "search_read", [[["product_tmpl_id", "=", item.productId]]], { fields: ["id"], limit: 1 });
        if (variant[0]) {
    ...
    const confirmUrl = `${baseUrl}/api/v1/manager-confirm/${order._id}`;
    const cancelUrl = `${baseUrl}/api/v1/manager-cancel/${order._id}`;
    const itemsWithNames = order.items.map((item) => item.toObject());
