export const toCleanProduct = (product, attributeValues) => {
    ...
    const attrs = hasTemplateId
        ...
        : attributeValues;
    const getAll = (type) => attrs
        .filter((a) => a.attribute_id?.[1]?.toLowerCase().includes(type))
        .map((a) => a.name);
    const getBrand = () => {
        const match = attrs.find((a) => a.attribute_id?.[1]?.toLowerCase() === "brand");
        return match?.name || null;
    };
    const XCD_RATES = { USD: 2.7, EUR: 2.9 };
    const currencyName = product.currency_id?.[1] || "USD";
    const detectedCurrency = Object.keys(XCD_RATES).find((k) => currencyName.toUpperCase().includes(k)) || "USD";
