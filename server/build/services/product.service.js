...
const mapProductInput = (p, supplierName, location) => ({
  ...
  x_item_number: p.x_item_number || "",
  x_shipping_cost: p.x_shipping_cost || 0,
  x_currency: p.x_currency || "USD",
});
...
export const getAllProductsService = async (category) => {
  ...
  const locationIds = [
    ...new Set(allQuants.map((q) => q.location_id?.[0]).filter(Boolean)),
  ];
...
export const checkOdooConnection = async () => {
  try {
    const result = await odooRequest("res.users", "search_read", [], {
      fields: ["id"],
      limit: 1,
    });
    return {
